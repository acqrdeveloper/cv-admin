...
class ExportRepo
{
    ...
    public function __construct()
    {
        ...
        $this->backgroundredstrong =  array(
            'fill' => array(
                'type' => \PHPExcel_Style_Fill::FILL_SOLID,
                'color' => array('rgb' => 'f02020')
            )
        );
        ...
        $this->backgroundexterno =  array(
            'fill' => array(
                'type' => \PHPExcel_Style_Fill::FILL_SOLID,
                'color' => array('rgb' => '9e7566')
            )
        );
    }
    ...
    public function Data_dashboard_empresasregistradas($objPHPExcel, $params)
    {
        ...
        $head = array( "Asesor", "Empresa", "Plan", "Ciclo", "Creacion", "Contrato", "Deuda", "Efectivo", "Deposito"  );
        $bodysql = array( "asesor", "empresa_nombre", "plan", "preferencia_facturacion", "fecha_creacion", "contrato", "deuda", "efectivo", "deposito"  );
        ...
        $sheet->getStyle('G2:I' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('G2:I' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
    ...
    public function Data_dashboard_empresahistorial($objPHPExcel, $params)
    {
        ...
        $head = array(    
            "Asesor", "Empresa",        "Fecha", "Contrato", "Empleado", "N DOC",       
            "Contrato", "R. Nombre",            "R. Correo",            "R. Telefono",
            "Ciclo", "Fiscal", "Garantia", "Deuda", "Seguimiento"
        );
        $bodysql = array( 
            "asesor", "empresa_nombre", "fecha", "contrato", "empleado", "empresa_ruc", 
            "contrato", "representante_nombre", "representante_correo", "representante_telefono",
            "ciclo", "fiscal", "garantia", "deuda", "seguimiento"
        );
        ...
        $sheet->getStyle('N2:N' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('N2:N' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
    ...
    public function Data_payment($objPHPExcel, $params)
    {
        ...
        $fila = 1;
        $col = -1;
        $head = array( 'Empresa', 'Comprobante', 'Numero', 'Creada', 'Pago', 'Tipo de pago', 'Monto', 'Detraccion', 'Monto Visa', 'Interes Visa', 'Creado Por' );
        $bodysql = array( 'empresa_nombre', 'comprobante', 'numero', 'fecha_emision', 'fecha_pago', 'tipo', 'monto_pago', 'detraccionD', 'dif_dep_pos', 'des_com_pos', 'usuario' );
    ...
    public function Data_sunatcomprobante($objPHPExcel, $params){
        $alpha = $this->alpha();
        $objPHPExcel->setActiveSheetIndex(0);
        $sheet = $objPHPExcel->getActiveSheet();
        ...
        $head = array( 'Mes', 'Factura', 'Boleta', 'Provicional', 'Credito', 'Debito' );
        $bodysql = array( 'mes', 'factura', 'boleta', 'provicional', 'credito', 'debito' );
        foreach ($head as $h) {
            $col = $col + 1;
            $coordinate = $alpha[$col] . $fila;
            $sheet->setCellValue($coordinate, strtoupper($h));
        ...
        $data = (new QueryRepo)->Q_reporte_sunatcomprobante($params);
        foreach ($data["rows"] as $r) {
            $r = (array)$r;
            $fila = $fila + 1;
            $col = -1;
        ...
        $sheet->getStyle('B2:F' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('B2:F' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet = $this->autoSize($sheet, $alpha, $alpha);
        return $objPHPExcel;  
    }

    public function Data_cuadremensual($objPHPExcel, $params){
        $alpha = $this->alpha();
        $objPHPExcel->setActiveSheetIndex(0);
        $sheet = $objPHPExcel->getActiveSheet();
        $sheet->setTitle('Cuadre');
        ...
        $head1 = array( "Empresa", "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Setiembre", "Octubre", "Noviembre", "Diciembre", "Total" );
        $head2 = array( "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago", "Deuda", "Pago" );

        $bodysql = array(
            "empresa_nombre",
            "01_d", "01_p", "02_d", "02_p", "03_d", "03_p", "04_d", "04_p",
            "05_d", "05_p", "06_d", "06_p", "07_d", "07_p", "08_d", "08_p",
            "09_d", "09_p", "10_d", "10_p", "11_d", "11_p", "12_d", "12_p",
            "d", "p"
        );
        ...
        foreach ($head1 as $h) {
            ...
            if( $col < 1 ){
                ...
                $sheet->mergeCells($alpha[$col] . $fila.':'.$alpha[$col+1] . $fila);
            }
        }
        $sheet->getStyle('A' . $fila . ':' . $alpha[$col] . $fila)->getFont()->setSize(10);
        $sheet->getStyle('A' . $fila . ':' . $alpha[$col] . $fila)->getFont()->setBold(true);
        ...
        $data = (new QueryRepo)->Q_reporte_cuadremensual($params);
        foreach ($data["rows"] as $r) {
            $r = (array)$r;
            $fila = $fila + 1;
            $col = -1;
            ...
            foreach ($bodysql as $b) {
                ...
                $coordinate = $alpha[$col] . $fila;
                $r[$b] != "" ? $sheet->setCellValue($coordinate, strtoupper($r[$b])) : "";
                if( $col > 0 && ( $col%2 == 0 ) && $deuda > $r[$b] ){
                    $sheet->getStyle( $alpha[$col] . $fila)->applyFromArray( $this->backgroundred );
        ...
        $sheet->getStyle('A3:' . $alpha[$col] . $fila)->applyFromArray($this->alignleft);
        $sheet->getStyle('B3:AD' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('B3:AD' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet = $this->autoSize($sheet, $alpha, $alpha);
        return $objPHPExcel;  
    }

    public function Data_localvisitantes($objPHPExcel, $params){
        $alpha = $this->alpha();
        $objPHPExcel->setActiveSheetIndex(0);
        $sheet = $objPHPExcel->getActiveSheet();
        $sheet->setTitle('Cuadre');
        $fila = 1;
        $col = -1;
        $head = array( 'Local', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11' ,'12', 'Total' );
        $bodysql = array( 'nombre', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11' ,'12', 'total' );
        foreach ($head as $h) {
            $col = $col + 1;
            $coordinate = $alpha[$col] . $fila;
            $sheet->setCellValue($coordinate, strtoupper($h));
        ...
        $data = (new QueryRepo)->Q_reporte_localvisitantes($params);
        foreach ($data["rows"] as $r) {
            $r = (array)$r;
            $fila = $fila + 1;
            $col = -1;
        ...
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getFont()->setSize(8);
        $sheet->getStyle('A2:' .'A' . $fila)->applyFromArray($this->alignleft);
        $sheet->getStyle('B2:F' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet = $this->autoSize($sheet, $alpha, $alpha);
        return $objPHPExcel;  
    ...
    public function Data_empresaactual($objPHPExcel, $params)
    {
        $alpha = $this->alpha();
        $objPHPExcel->setActiveSheetIndex(0);
        $sheet = $objPHPExcel->getActiveSheet();
        ...
        $fila = 1;
        $col = -1;
        $head = array( 'Nombre', 'Estado', 'Activo', 'Pendiente', 'Suspendido', 'Eliminado', 'Total', 'Act. con Convenio', 'Act. sin Convenio' );
        $bodysql = array( 'nombre', 'estado', 'activo', 'pendiente', 'suspendido', 'eliminado', 'total', 'activo_conconvenio', 'activo_sinconvenio' );
        ...
        $data = (new QueryRepo)->Q_reporte_empresaactual($params);
        foreach ($data["rows"] as $r) {
            $r = (array)$r;
            $fila = $fila + 1;
            $col = -1;
        ...
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getFont()->setSize(8);
        $sheet->getStyle('B2:' .'B' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('C2:I' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('C2:I' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet = $this->autoSize($sheet, $alpha, $alpha);
        $sheet->setAutoFilter('A1:'.$alpha[$col] .'1');
        return $objPHPExcel;        
    ...
    public function Data_facturanualreporte($objPHPExcel, $params)
    {
        ...
        foreach( $meslist as $key => $m ){
            ...
            foreach ( $sheetmes[$m] as $r) {
                ...
                if( $r["estado"] == 'ANULADA' ){
                    $sheet->getStyle('C'. $fila)->applyFromArray( $this->backgroundredstrong );
                }
            }
    ...
    public function Data_coffeebreak($objPHPExcel, $params)
    {
        $alpha = $this->alpha();
        $objPHPExcel->setActiveSheetIndex(0);
        $sheet = $objPHPExcel->getActiveSheet();
        ...
        $fila = 1;
        $col = -1;
        $head = array( 'Empresa', 'Sede', 'Oficina', 'Piso', 'Fecha', 'Hora Inicio', 'Hora Fin', 'Combo', 'Nro. Personas' );
        $bodysql = array( 'empresa_nombre', 'local_nombre', 'nombre', 'piso_id', 'fecha_reserva', 'hora_inicio', 'hora_fin', 'concepto_nombre', 'cantidad' );
        ...
        $data = (new QueryRepo)->Q_reporte_coffeebreak($params);
        foreach ($data["rows"] as $r) {
            $r = (array)$r;
            $fila = $fila + 1;
            $col = -1;
        ...
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getFont()->setSize(8);
        $sheet->getStyle('B2:' .'G' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('H2:' .'H' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet = $this->autoSize($sheet, $alpha, $alpha);
        return $objPHPExcel;
    }

    public function Data_reserva($objPHPExcel, $params)
    {
        $alpha = $this->alpha();
        $objPHPExcel->setActiveSheetIndex(0);
        $sheet = $objPHPExcel->getActiveSheet();
        ...
        $fila = 1;
        $col = -1;
        $head = array( 'Estado', 'F de Reserva', 'Ini', 'Fin', 'Tiempo', 'Local', 'Modelo', 'Tipo', 'Oficina', 'Piso', ' N. Doc', 'empresa', 'Cochera', 'Placa', 'Audio', 'Evento', 'Mesa', 'Silla', 'Limpieza', 'Creado por', 'Created enl' );
        $bodysql = array( 'estado', 'fecha_reserva', 'hora_inicio', 'hora_fin', 'diferencia', 'local_nombre', 'modelo_nombre', 'oficina_tipo', 'oficina_nom', 'piso_id', 'empresa_ruc', 'empresa_nombre', 'cochera_nombre', 'placa', 'audio', 'evento_nombre', 'mesa', 'silla', 'limpieza', 'creado_por', 'created_at' );
        ...
        $data = (new QueryRepo)->Q_reserva($params);
        ...
        foreach ($data["rows"] as $r) {
            ...
            if( $r["estado"] == 'E' ){
                ...
                $sheet->getStyle('A'. $fila)->applyFromArray( $this->backgroundexterno );
            }
        }
        
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getFont()->setSize(8);
        $sheet->getStyle('A2:' .'K' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('M2:' .'P' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('Q2:' .'U' . $fila)->applyFromArray($this->aligncenter);
        /*
        $sheet->getStyle('C2:' .'J' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('L2:' .'P' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('Q2:' .'Q' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('Q2:' .'Q' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('S2:' .'V' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('S2:' .'V' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        */
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet = $this->autoSize($sheet, $alpha, $alpha);
        return $objPHPExcel;
    ...
    public function Data_auditorio($objPHPExcel, $params)
    {
        $alpha = $this->alpha();
        $objPHPExcel->setActiveSheetIndex(0);
        $sheet = $objPHPExcel->getActiveSheet();
        ...
        $fila = 1;
        $col = -1;
        $head = array( 'RUC', 'Empresa', 'Sede', 'Oficina', 'Piso', 'Creada En', 'Programada Para', 'Hora Inicio', 'Hora Fin', 'Tiempo', 'Evento', 'Comprobante', 'Numero', 'Monto', 'Estado', 'Efectivo', 'Deposito', 'Auditorio', 'Coffee Break', 'Estado Reserva' );
        $bodysql = array( 'empresa_ruc', 'empresa_nombre', 'local_nombre', 'nombre', 'piso_id', 'created_at', 'fecha_reserva', 'hora_inicio', 'hora_fin', 'tiempo', 'evento_nombre', 'factura_comprobante', 'factura_numero', 'factura_monto', 'factura_estado', 'efectivo', 'deposito', 'audiotorio_monto' ,'break_monto', 'reserva_estado' );
        ...
        $data = (new QueryRepo)->Q_reporte_auditorio($params);
        foreach ($data["rows"] as $r) {
            $r = (array)$r;
            $fila = $fila + 1;
            $col = -1;
        ...
        $sheet->getStyle('A2:' .'A' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('C2:' .'J' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('L2:' .'M' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('N2:' .'N' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('N2:' .'N' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('O2:' .'O' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('P2:' .'S' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('P2:' .'S' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('U2:' .'U' . $fila)->applyFromArray($this->aligncenter);
        $sheet->getStyle('A2:' . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet = $this->autoSize($sheet, $alpha, $alpha);
        return $objPHPExcel;
    }
    ...
    public function Data_empresa($objPHPExcel, $params)
    {
        ...
        $head = array('Estado', 'Empresa', 'Comercial', 'Ruc', 'Encargado', 'Central', 'Plan', 'Ciclo', 'Fact. Nombre', 'Fact. Email', 'Fact. Tel.', 'Fact. Cel.', 'Representante', 'Rep. Email', 'Total Deuda');
        $bodysql = array('preferencia_estado', 'empresa_nombre', 'nombre_comercial', 'empresa_ruc', 'encargado', 'central', 'plan', 'preferencia_facturacion', 'encargado', 'fac_email', 'fac_telefono', 'fac_celular', 'representante_nombre', 'representante_email', 'deutatotal' );
        ...
        foreach ($data["rows"] as $r) {
            ...
            if( $r["preferencia_estado"] == 'E' ){
                ...
                $sheet->getStyle('A'. $fila)->applyFromArray( $this->backgroundredpendiente );
            }else if( $r["plan_id"] == '31' ){
                $sheet->getStyle('A'. $fila)->applyFromArray( $this->backgroundexterno );
            }


            if( $r["deutatotal"] > 0  ){
                $sheet->getStyle('O'. $fila)->applyFromArray( $this->backgroundred );
            }
        }
        ...
        $sheet->getStyle('O2:O' . $fila)->applyFromArray($this->alignright);
        $sheet->getStyle('O2:O' . $fila)->getNumberFormat()->setFormatCode("#,##0.00");
        $sheet->getStyle('A2:'  . $alpha[$col] . $fila)->getFont()->setSize(8);
        $sheet->getStyle('A2:'  . $alpha[$col] . $fila)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $sheet->getStyle('K2:L' . $fila)->applyFromArray($this->aligncenter);
        $sheet->setAutoFilter('A1:'.$alpha[$col] .'1');
